// Generate Module 5 audio via OpenAI tts-1-hd.
// Reads manifest, calls /v1/audio/speech per trigger, writes mp3s,
// appends per-file rows to audio-cost-log.csv.
// Pricing: tts-1-hd = $30 per 1M chars.
...
async function generateOne(trigger, apiKey) {
  ...
  return { chars, cost, bytes: buf.length, outAbs };
}
...
async function main() {
  ...
  const triggers = manifest.ttsTriggers.filter((t) => t.mvpRequired);
  ...
  for (const t of triggers) {
    try {
      ...
      totalChars += r.chars;
      totalCost += r.cost;
      const row = `${today},5,openai-tts-1-hd,${r.chars},${r.cost.toFixed(6)},${t.outputFile}\n`;
      fs.appendFileSync(COST_LOG, row);
      results.push({ id: t.id, ok: true, chars: r.chars, bytes: r.bytes });
      console.log(`ok  ${t.id} chars=${r.chars} bytes=${r.bytes}`);
    } catch (e) {
      failures += 1;
      results.push({ id: t.id, ok: false, err: e.message });
      console.error(`ERR ${t.id} :: ${e.message}`);
    }
  }

  const summary = {
    triggersAttempted: triggers.length,
    succeeded: triggers.length - failures,
    failed: failures,
    totalChars,
