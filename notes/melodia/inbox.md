...
> Write anything here that Claude needs to see at the next session start.
> Claude reads this, processes everything, then clears it.
> Drop images in `design/` if you want Claude to look at something visual.

<!-- Add your notes below this line -->
the sound is not great on screen for the lesson with letters it just sounds robotic and is not informational

I sort of want something that is even bigger, so I think this is a bigger conversation that needs to happen, a bigger project. I need voices in the app that will actually break down each of these rectangle. So for example, let me just give you an example. It's talking about the letter N with a tilde on top of it, so it's like enye. The sound is like niño, mañana, año, you know? So it needs to actually give examples. So it would be like, like it will say it in English so you understand, and then it will translate it to Spanish, and then give an example in Spanish. So it'll be like, example words with this letter are niño, mañana, año, you know? So it'll break down each of these rectangles, you know, highlighting each word if possible as it goes or something. I don't know, maybe that's not right, this is a larger project because I want the voice to not be robotic, so I don't know if we need to introduce AI into this, so that, um, like it's like an AI that will take care of this, and it will have clear instructions, so I would need like an API key or something to add in the app so that it can handle this and make it faster, or we need to engineer this sort of from scratch without AI. But the thing is, the voice is not good. It's just not good. We need an original, nice, clear looking voice, like the way I, you get what I'm saying, like an actual voice that's like sounds human, not some robotic voice, so we need something better. Are there existing systems in place, other applications that we could connect to this app to make this a reality with the fewest steps possible? I need to brainstorm and do research on how this can be done because we not only need to just read out what's being said, but we also need to give broader context. So English, switch to English smoothly to Spanish smoothly in a real human voice that explains the concept in structured phrases, not just just saying the letters. Does that make sense?


when the song was done, i didnt get a banner notification wheni was in youtube music telling me the timer is done and to coe back to the melodia app, are you sure this has been created? the expo notifications are on sooo....


also i didn't catch it but does the timer always match the leght of the song in the link thats embeded ? or is it still a rough estimation

i found another bug, when i took the preposition quiz, it told me no quiz was availabe for this module yet, but then i tried to go back to the modules page and it just took me back to the page that said no quiz was availabe for this module yet. its not ltting me browse all 60 modules at this stage


also some songs are wrong, the song donde jugaran los ninos by mana wasnt thtt for youtube music![[Screenshot 2026-05-14 at 1.04.46 AM.png]]

Please redo the prompt to give Claude, but focus only on the agent team. So make sure this is thorough enough, so don't only, like, give the roles, but only elaborate on what they do, similar to how you did above. Really make it really thorough. I like, you know, the goal is to eventually build three lessons per day, but we will ramp and start week one, one lesson, week two, two lessons, etc. I like that. But yeah, definitely build on each of the, you know, agents, their role and how they work, what their deliverables are. I think that's all really important. Also, if you can also mention to definitely not only consider this prompt that I'm giving, but also the notes that are in the docs folder, like the agents team reference. Definitely make sure to use this in parallel with this prompt so that you know how to make this team work well within this project that we have. We also want to make the permissions so that it doesn't, it's free to, like, I want within this project, when agents are in action, that they can just act autonomously without having to prompt me in the terminal, so I want permissions to be more lax. So I need to do that in the settings. If you can let me know how to do that or do it yourself, that'd be great. What else? Yeah. So can you update this prompt to have all this? It needs to be very thorough, so think deep.I don't know if this is the leads or the orchestrator agent job, but I think that needs to be at the start of every lesson building session that we need to be in plan mode before we execute anything, right? We need to be in ultra plan, plan everything, and then execute because this is really important. So definitely include that in the prompt as well that it follows this, or maybe it already knows to do that. I don't know. Very important. let me know if anything needs to be done before we put the team in action 
help me set up tmox version to see the agents also there are too many agebts lets keep it to 6-7 agents so combine some of them together
